...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /f/koalalink/koalachain/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /f/koalalink/koalachain/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /f/koalalink/koalachain/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /f/koalalink/koalachain/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /f/koalalink/koalachain/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /f/koalalink/koalachain/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /f/koalalink/koalachain/linux-build
make check

# Run RPC integration test on Linux:
/f/koalalink/koalachain/qa/rpc-tests/wallet.sh /f/koalalink/koalachain/linux-build/src
/f/koalalink/koalachain/qa/rpc-tests/listtransactions.py --srcdir /f/koalalink/koalachain/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /f/koalalink/koalachain/win32-build
  make check
fi
