...
#define ADDR_SEND_A        "dcmCbKbAfKrofNz35MSFupxrx4Uwn3vgjL"
// 1826-1437

#define ADDR_ACCEPT_B      "dcmWdcfxEjRXUHk8LpygtgDHTpixoo3kbd"
// 1826-1285

#define ADDR_ARBITRATION_C "dcnGLkGud6c5bZJSUghzxvCqV45SJEwRcH"
// 1826-1081

//const static char DeveloperAccount[6]="\x00\x00\x00\x00\x14\x00";//!<
...
//#define ID_strAppRegId  "47301-1"
//#define ID_strAppRegId  "47322-1"
//#define ID_strAppRegId  "47323-1"
#define ID_strAppRegId  "47018-1"    //
//#define HASH_sendhash     "7de1faafc2c9f14be5294f5f2b1082eaf92c7d66da5d42be1016e0988143318d"
...
#define ARBITER_arbiterMoneyX      (2 * 1000000)
#define ARBITER_overtimeMoneyYmax  (1 * 100000000)
#define ARBITER_configMoneyZ       (1 * 1000000)
#define ARBITER_overtimeheightT    (1 * 1440)


#define SEND_moneyM     (2 * 100000000)
#define SEND_height     (2 * 1440)

//#define  ARBITER_winnerMoney  (1 * 100000000)
#define  ARBITER_winnerMoney  (198000000)
...
//	senddata.sendType = SEND_TYPE_BUY;
	senddata.sendType = SEND_TYPE_SELL;

//	string arbitationID = "47046-1";
	unsigned int height = 1826;
	...
	strcpy(senddata.goods,"11");
	strcpy(senddata.comment,"13418656754");
    ...
    {
    	sendret= basetest.CreateContractTx(strAppRegId,ADDR_SEND_A,sendcontract,0,0,nTempSend);
    }else{
    	sendret= basetest.CreateContractTx(strAppRegId,ADDR_ACCEPT_B,sendcontract,0,0,nTempSend);
    }
...
//	memcpy(senddata.txhash, uint256S(HASH_sendhash).begin(), sizeof(senddata.txhash));
	memcpy(senddata.txhash,HASH_sendhash, sizeof(senddata.txhash));
	senddata.height = SEND_height;
	...
	Value  sendret= basetest.CreateContractTx(strAppRegId,ADDR_SEND_A,sendcontract,0,0,nTempSend);
//    Value  sendret= basetest.CreateContractTx(strAppRegId,ADDR_ACCEPT_B,sendcontract,0,0,nTempSend);
	if (basetest.GetHashFromCreatedTx(sendret, strTxHash)) {
		nStep++;
	}
	else{
...
bool CGuaranteeTest::RunFinalResult()
{
   cout<<"RunFinalResult start"<<endl;
	...
	memcpy(senddata.arbitHash, HASH_sendhash, sizeof(senddata.arbitHash));
	senddata.overtimeheightT = ARBITER_overtimeheightT;
...
//	string sellerId = "1826-1285";
	...
	senddata.loserMoney = SEND_moneyM - ARBITER_winnerMoney;
