...
boost::filesystem::path GetDefaultDataDir() {
	...
	// Windows < Vista: C:\Documents and Settings\Username\Application Data\koala
	// Windows >= Vista: C:\Users\Username\AppData\Roaming\koala
	// Mac: ~/Library/Application Support/koala
	// Unix: ~/.koala
#ifdef WIN32
	// Windows
	return GetSpecialFolderPath(CSIDL_APPDATA) / "koala";
#else
	fs::path pathRet;
...
boost::filesystem::path GetConfigFile() {
	boost::filesystem::path pathConfigFile(CBaseParams::GetArg("-conf", "koalachain.conf"));
...
void ReadConfigFile(map<string, string>& mapSettingsRet, map<string, vector<string> >& mapMultiSettingsRet) {
	...
	if (!streamConfig.good())
		return; // No koalachain.conf file is OK

	set<string> setOptions;
	setOptions.insert("*");

	for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
		// Don't overwrite existing settings so command line settings override koalachain.conf
			...
			if (!fDone) {
				...
				if (!fMatch) {
					...
					string strMessage = _("Warning: Please check that your computer's date and time "
							"are correct! If your clock is wrong koalad will not work properly.");
