...
namespace {
class CKoalaAddressVisitor: public boost::static_visitor<bool> {
private:
	CKoalaAddress *addr;
public:
	CKoalaAddressVisitor(CKoalaAddress *addrIn) :
			addr(addrIn) {
	}
...
bool CKoalaAddress::Set(const CKeyID &id) {
...
bool CKoalaAddress::Set(const CTxDestination &dest) {
	return boost::apply_visitor(CKoalaAddressVisitor(this), dest);
}

bool CKoalaAddress::IsValid() const {
...
CTxDestination CKoalaAddress::Get() const {
	if (!IsValid())
...
bool CKoalaAddress::GetKeyID(CKeyID &keyID) const {
	uint160 id;
...
//bool CKoalaAddress::GetRegID(CRegID &Regid) const {
//
////	vector<unsigned char> vid;
////	vid.push_back(CBaseParams::ACC_ADDRESS);
...
bool CKoalaAddress::IsScript() const {
	return IsValid() && vchVersion == SysCfg().Base58Prefix(CBaseParams::SCRIPT_ADDRESS);
}

void CKoalaSecret::SetKey(const CKey& vchSecret) {
...
CKey CKoalaSecret::GetKey() {
	CKey ret;
	ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
	return ret;
}

bool CKoalaSecret::IsValid() const {
	bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
	bool fCorrectVersion = vchVersion == SysCfg().Base58Prefix(CBaseParams::SECRET_KEY);
	return fExpectedFormat && fCorrectVersion;
}

bool CKoalaSecret::SetString(const char* pszSecret) {
	return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CKoalaSecret::SetString(const string& strSecret) {
