...
//
#ifndef KOALA_BASE58_H
#define KOALA_BASE58_H
...
/** base58-encoded Koala addresses.
 * Public-key-hash-addresses have version 0 (or 111 testnet).
 * The data vector contains RIPEMD160(SHA256(pubkey)), where pubkey is the serialized public key.
 * Script-hash-addresses have version 5 (or 196 testnet).
 * The data vector contains RIPEMD160(SHA256(cscript)), where cscript is the serialized redemption script.
 */
class CKoalaAddress : public CBase58Data {
public:
    ...
    CKoalaAddress() {}
    CKoalaAddress(const CTxDestination &dest) { Set(dest); }
    CKoalaAddress(const string& strAddress) { SetString(strAddress); }
    CKoalaAddress(const char* pszAddress) { SetString(pszAddress); }
 ...
 */
class CKoalaSecret : public CBase58Data
{
public:
    ...
    CKoalaSecret(const CKey& vchSecret) { SetKey(vchSecret); }
    CKoalaSecret() {}
};

template<typename K, int Size, CBaseParams::Base58Type Type> class CKoalaExtKeyBase : public CBase58Data
{
public:
...
typedef CKoalaExtKeyBase<CExtKey, 74, CBaseParams::EXT_SECRET_KEY> CKoalaExtKey;
typedef CKoalaExtKeyBase<CExtPubKey, 74, CBaseParams::EXT_PUBLIC_KEY> CKoalaExtPubKey;

#endif // KOALA_BASE58_H
