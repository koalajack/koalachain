...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest)
{
    ...
    // Connect
    SOCKET hSocket;
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, SysCfg().GetDefaultPort()) : ConnectSocket(addrConnect, hSocket))
    {
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Koala users should be well used to small numbers with many decimal places by now :)
...
void static StartSync(const vector<CNode*> &vNodes) {
    ...
    for (auto pnode : vNodes) {
        ...
        if (!pnode->fClient && !pnode->fOneShot &&
            ...
            (pnode->nStartingHeight > (nBestHeight - 144)) /*&& (pnode->nVersion < NOBLKS_VERSION_START || pnode->nVersion >= NOBLKS_VERSION_END)*/
            ) {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Koala Core is probably already running."), addrBind.ToString());
        else
