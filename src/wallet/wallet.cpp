...
bool GetBlockByTxHash(const uint256& txhash, int& txIndex) {
	...
	txIndex = -1;
	{
		...
		ReadBlockFromDisk(block, pgenesisblockindex);
		assert(block.GetHashMerkleRoot() == block.BuildMerkleTree());
		for (unsigned int i = 0; i < block.vptx.size(); ++i) {
			if (txhash == block.GetTxHash(i)) {
				txIndex = i;
			}
		}
		...
		if (SysCfg().IsTxIndex()) {
			...
			if (pScriptDBTip->ReadTxIndex(txhash, postx)) {
				...
				try {
					file >> header;
					CBlockIndex* pblockindex = chainActive[header.GetHeight()];
