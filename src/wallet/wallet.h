...
#ifndef KOALA_WALLET_H
#define KOALA_WALLET_H
...
struct CmpByBlockHeight {
	  bool operator()(const uint256& k1, const uint256& k2) const {
		  CBlockIndex *pblockindexK1 = NULL, *pblockindexK2 = NULL;
		  if(mapBlockIndex.count(k1) > 0)  {
			  pblockindexK1 = mapBlockIndex[k1];
		  } else{
			  return false;
		  }
		  if(mapBlockIndex.count(k2) > 0) {
			  pblockindexK2 = mapBlockIndex[k2];
		  } else {
			  return false;
		  }
		  return pblockindexK1->nHeight > pblockindexK2->nHeight;
	  }
	};
...
extern bool GetBlockByTxHash(const uint256& txhash, int& txIndex);

struct CmpByTxIndex {
	  bool operator()(const uint256& tx1, const uint256& tx2) const {
		  int txIndex1 = -1;
		  int txIndex2 = -1;
		  GetBlockByTxHash(tx1, txIndex1);
		  if(txIndex1 < 0)
			  return false;
		  GetBlockByTxHash(tx2, txIndex2);
		  if(txIndex2 < 0)
			  return false;
