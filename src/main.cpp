...
#if defined(NDEBUG)
# error "Koala cannot be compiled without assertions."
#endif
...
int g_nSyncTipHeight(0);  //chainActive.Tip()->nHeight
...
const string strMessageMagic = "Koala Signed Message:\n";
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp) {
	...
	if (hash != SysCfg().HashGenesisBlock()) {
		...
		if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast())
			return state.Invalid(ERRORMSG("AcceptBlock() : block's timestamp is too early"), REJECT_INVALID,
					"time-too-old");
...
#define MAX_SUBSIDY_HEIGHT 15768000
