...
void Shutdown()
{
    ...
    RenameThread("Koala-shutoff");
    mempool.AddTransactionsUpdated(1);
    StopRPCThreads();
    ShutdownRPCMining();


    GenerateKoalaBlock(false, NULL, 0);
...
string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: koalachain.conf)") + "\n";
    if (hmm == HMM_BITCOIND)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: koalad.pid)") + "\n";
    ...
    strUsage += "\n" + _("RPC SSL options: (see the Koala Wiki for SSL setup instructions)") + "\n";
...
//    COINBASE_FLAGS << vector<unsigned char>(pszP2SH, pszP2SH+strlen(pszP2SH));
    ...
    // Make sure only a single Koala process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Koala Core is probably already running."), strDataDir));

//    if (GetBoolArg("-shrinkdebugfile", !fDebug))
//        ShrinkDebugFile();

    LogPrint("INFO","\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrint("INFO","Koala version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE);
    printf("Koala version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
...
//                cout <<pScriptDB->GetDbCount() <<endl;
	...
	if (pwalletMain) {
		GenerateKoalaBlock(SysCfg().GetBoolArg("-gen", false), pwalletMain, SysCfg().GetArg("-genproclimit", -1));
