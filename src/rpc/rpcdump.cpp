...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"koalaprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"koalaprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"koalaaddress\"\n"
            "\nReveals the private key corresponding to 'koalaaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"koalaaddress\"   (string, required) The Koala address for the private key\n"
            "\nResult:\n"
    ...
    CKoalaAddress address;
    if (!address.SetString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Koala address");
    CKeyID keyID;
    ...
    if (!pwalletMain->GetKey(keyID, minerkey,true))
           throw JSONRPCError(RPC_WALLET_ERROR, "Private key for address " + strAddress + " is not known");
    Object reply;
    	reply.push_back(Pair("privkey", CKoalaSecret(vchSecret).ToString()));
    if(minerkey.ToString() != vchSecret.ToString())
    	reply.push_back(Pair("minerkey", CKoalaSecret(minerkey).ToString()));
    else
...
Value dumpwallet(const Array& params, bool fHelp) {
	...
	Object reply;
	reply.push_back(Pair("created by Koala", CLIENT_BUILD + CLIENT_DATE));
