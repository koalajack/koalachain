...
Value listtransactions(const Array& params, bool fHelp) {
	...
	for (auto const &wtx : pwalletMain->mapInBlockTx) {
		for (auto const & item : wtx.second.mapAccountTx) {
			...
			if(item.second->nTxType == COMMON_TX) {
				vKeyIdSet.clear();
				item.second->GetAddress(vKeyIdSet, *pAccountViewTip, *pScriptDBTip);
				std::set<CKeyID>::iterator iterRecvKey = vKeyIdSet.begin();
				++iterRecvKey;
				if ("*" != strAddress && iterRecvKey->ToAddress() != strAddress) {
					continue;
				}
				...
				if(pwalletMain->HaveKey(*iterRecvKey)) {
					obj.push_back(Pair("address", iterRecvKey->ToAddress()));
					obj.push_back(Pair("category", "receive"));
				}
	...
	return arrayData;
	*/
	return listtransactionsv2(params, fHelp);
...
Value validateaddress(const Array& params, bool fHelp)
{
	if(fHelp || params.size() != 1)
		{
			throw runtime_error(
					 "isvalideaddess \"koala address\"\n"
					 "\ncheck address is valide\n"
					 "\nArguments:\n"
					 "1. \"koala address\"  (string, required) koala address\n"
					 "\nResult:\n"
					 "\nExamples:\n"
		...
		}
	{
		...
		if (!GetKeyId(addr, keyid)) {
			...
			obj.push_back(Pair("isvalid" , true));
		}
		return obj;
	}
}
