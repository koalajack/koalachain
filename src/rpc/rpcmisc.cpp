...
Value getbalancev2(const Array& params, bool fHelp)
{
	int size = params.size();
		if (fHelp || params.size() > 2) {
				string msg = "getbalanceV2 ( \"address\" minconf )\n"
						...
						"\nArguments:\n"
						 ...
						 "\nExamples:\n"
						+ HelpExampleCli("getbalanceV2", "de3nGsPR6i9qpQTNnpC9ASMpFKbKzzFLYF 0")
						+ "\nAs json rpc call\n"
						+ HelpExampleRpc("getbalanceV2", "de3nGsPR6i9qpQTNnpC9ASMpFKbKzzFLYF 0");
				throw runtime_error(msg);
		}
		Object obj;
		if (size == 0) {
			return ValueFromAmount(pwalletMain->GetRawBalance());
		} else if (size == 1) {
			...
			if (addr == "*") {
				...
				if (pwalletMain->HaveKey(keyid)) {
					...
					if (accView.GetAccount(CUserID(keyid), account)) {
						return ValueFromAmount(account.GetRawBalance());
					}
				} else {
			...
			if (addr == "*") {
				if (0 != nConf) {
					...
					return ValueFromAmount(pwalletMain->GetRawBalance() - nValue);
				} else {
					return ValueFromAmount(pwalletMain->GetRawBalance(false));
				}
			} else {
				CKeyID keyid;
				if (!GetKeyId(addr, keyid)) {
				...
				if (pwalletMain->HaveKey(keyid)) {
					if (0 != nConf) {
						...
						return ValueFromAmount(pAccountViewTip->GetRawBalance(keyid) - nValue);
					} else {
						return ValueFromAmount(mempool.pAccountViewCache->GetRawBalance(keyid));
					}
				} else {
					throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "address not inwallet");
				}
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"Koalaaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"Koalaaddress\"  (string, required) The Koala address to use for the signature.\n"
