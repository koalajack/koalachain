...
Value getnewaddressv2(const Array& params, bool fHelp) {
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"koalaaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"koalaaddress\"  (string, required) The Koala address to use for the private key.\n"
...
Value sendtoaddresswithfee(const Array& params, bool fHelp)
 {
	...
	if (fHelp || (!(size == 3 || size == 4)))
		throw runtime_error(
						...
						"\nArguments:\n"
						"1. \"sendaddress\"  (string, optional) The Koala address to send to.\n"
						"2. \"recvaddress\" (string, required) The Koala address to receive.\n"
...
Value sendtoaddressraw(const Array& params, bool fHelp)
{
	...
	if (fHelp || size < 4 || size > 5 )
		throw runtime_error(
						...
						"3. \"srcaddress\"  (string, required) The Koala address to send to.\n"
						"4. \"recvaddress\"  (string, required) The Koala address to receive.\n"
...
Value sendtoaddressv2(const Array& params, bool fHelp)
 {
	int size = params.size();
	if (fHelp || (size != 3))
		throw runtime_error(
						"sendtoaddress (\"Koalaaddress\") \"receive address\" \"amount\"\n"
						"\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
						+ HelpRequiringPassphrase() + "\nArguments:\n"
						"1. \"Koalaaddress\"  (string, optional) The Koala address to send to.\n"
						"2. amount   (string, required) The Koala address to receive\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
 {
	...
	if (fHelp || (!(size == 2 || size == 3)))
		throw runtime_error(
						...
						+ HelpRequiringPassphrase() + "\nArguments:\n"
						"1. \"Koalaaddress\"  (string, optional) The Koala address to send to.\n"
						"2. receive address   (string, required) The Koala address to receive\n"
						"3.\"amount\"   (string, required) \n"
						"\nResult:\n"
						"\"transactionid\"  (string) The transaction id.\n"
						"\nExamples:\n"
...
Value walletpassphrasev2(const Array& params, bool fHelp)
{
	  if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
	        throw runtime_error(
	            "walletpassphrase \"passphrase\" timeout\n"
	            "\nStores the wallet decryption key in memory for 'timeout' seconds.\n"
	            "This is needed prior to performing transactions related to private keys such as sending Koalas\n"
	            "\nArguments:\n"
...
Value walletlockv2(const Array& params, bool fHelp)
{
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending Koala\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"Koalaaddress\" \"test message\"") +
...
//    boost::filesystem::rename(GetDataDir() / strFileCopy, GetDataDir() / defaultFilename);
    ...
    return retObj;
    //return "wallet encrypted; Koala server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value getwalletinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Koala balance of the wallet\n"
