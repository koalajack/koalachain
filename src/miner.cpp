		...
		if (pindex->nHeight <= nRegAppFuel2FeeForkHeight) {
			...
			if (nAverateStep < MAX_BLOCK_RUN_STEP * 3 / 4 /* 0.75 */) {
				...
				if (newFuelRate <= pindex->nFuelRate) {
					newFuelRate = pindex->nFuelRate + 1;
				}
				LogPrint("HeighStep", "Height: %d, AverateStep: %ld\n", pindex->nHeight, nAverateStep);
			} else {
				newFuelRate = pindex->nFuelRate;
...
//						acctInfo.keyID.ToString(), acctInfo.GetAccountPos(pBlock->nHeight), acctInfo.IsMiner(pBlock->nHeight));
				if (acctInfo.IsRegister() && acctInfo.GetAccountPos(pBlock->GetHeight()) > 0
						&& acctInfo.IsMiner(pBlock->GetHeight())) {
					if (SysCfg().NetworkID() == CBaseParams::MAIN) {
						if(acctInfo.GetRawBalance() < 1000 * COIN) {
							continue;
						}
					}
		...
		for (; nNonce < maxNonce; ++nNonce) {
			...
			if (UintToArith256(curhash) <= UintToArith256(adjusthash)) {
				...
				if (pAccountViewTip->GetRegId(item.keyID, regid)) {
					...
					if (pwalletMain->Sign(item.keyID, pBlock->SignatureHash(), vSign, item.MinerPKey.IsValid())) {
						LogPrint("INFO", "Create new block hash:%s\n", pBlock->GetHash().GetHex());
						LogPrint("miner", "Miner account info:%s\n", item.ToString());
						LogPrint("miner", "CreatePosTx block hash:%s, postxinfo:%s\n", pBlock->GetHash().GetHex(),
								postxinfo.ToString().c_str());
...
bool VerifyPosTx(CAccountViewCache &accView, const CBlock *pBlock, CTransactionDBCache &txCache,
...
//				LogPrint("ERROR", "signature block:%s\n", HexStr(pBlock->vSignature.begin(), pBlock->vSignature.end()));
	...
	if (prtx->nVersion != nTxVersion2) {
		return ERRORMSG(
				"CTransaction CheckTransction,tx version is not equal current version, (tx version %d: vs current %d)",
...
CBlockTemplate* CreateNewBlock(CAccountViewCache &view, CTransactionDBCache &txCache, CScriptDBViewCache &scriptCache) {
	...
	int64_t nFees = 0;
	{
		...
		while (!vecPriority.empty()) {
			...
			pBaseTx->nFuelRate = pblock->GetFuelRate();
			if (!pBaseTx->ExecuteTx(nBlockTx + 1, viewTemp, state, txundo, pIndexPrev->nHeight + 1, txCache,
					scriptCacheTemp)) {
				continue;
			}
			// Run step limits
		...
		((CRewardTransaction*) pblock->vptx[0].get())->rewardValue = nFees - nTotalFuel
				+ GetBlockSubsidy(pIndexPrev->nHeight + 1);
...
//	LogPrint("INFO","proof-of-work found  \n  hash: %s  \ntarget: %s\n", hash.GetHex(), hashTarget.GetHex());
	...
	// Found a solution
	{
		...
		if (pblock->GetHashPrevBlock() != chainActive.Tip()->GetBlockHash())
			return ERRORMSG("KoalaMiner : generated block is stale");
...
//		}
		...
		if (!ProcessBlock(state, NULL, pblock))
			return ERRORMSG("KoalaMiner : ProcessBlock, block not accepted");
	}

	return true;
}

bool static MiningBlock(CBlock *pblock, CWallet *pwallet, CBlockIndex* pindexPrev,
		unsigned int nTransactionsUpdatedLast, CAccountViewCache &view, CTransactionDBCache &txCache,
...
void static KoalaMiner(CWallet *pwallet, int targetConter) {
	LogPrint("INFO", "Miner started\n");

	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("Koala-miner");
	...
	auto CheckIsHaveMinerKey = [&]() {
	 ...
	 LogPrint("INFO", "KoalaMiner  terminated\n");
...
void GenerateKoalaBlock(bool fGenerate, CWallet* pwallet, int targetHigh) {
...
//		assert(0);
	...
	minerThreads->create_thread(boost::bind(&KoalaMiner, pwallet, targetHigh));
