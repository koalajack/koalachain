...
bool CScriptDBViewCache::GetScript(const int nIndex, vector<unsigned char> &vScriptId, vector<unsigned char> &vValue) {
	if (0 == nIndex) {
		...
		if (!pBase->GetScript(nIndex, vScriptId, vValue)) {
			if (vDataKey.empty())
				return false;
			else {
				vScriptId.clear();
			...
			dataKeyTemp.insert(dataKeyTemp.end(), vScriptId.begin(), vScriptId.end());
			if (dataKeyTemp < vDataKey) {
				if (mapDatas.count(dataKeyTemp) == 0)
					return true;
				else {
					mapDatas[dataKeyTemp].clear();
					return GetScript(nIndex, vScriptId, vValue);
				}
			} else {
				vScriptId.clear();
				vValue.clear();
